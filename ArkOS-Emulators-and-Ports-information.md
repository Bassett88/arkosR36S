...
- To get Ocean's Heart running, you'll need to edit `scripts\menus\map_banner.lua` and comment out the line `local swipe_fade = require"scripts/fx/swipe_fade"` in the .solarus file provide.  You'll also need to edit `lone_windmill.lua`, `pirate_fort.lua` and `west_trail.lua` in `maps/fykonos/` and commenting out the line that includes `sol.shader.create`.  This will stop the game from crashing.  Thanks to Romadu for the research in finding this fix.
